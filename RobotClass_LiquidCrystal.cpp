...
RobotClass_LiquidCrystal::RobotClass_LiquidCrystal(uint8_t rs, uint8_t enable, uint8_t d0, uint8_t d1, uint8_t d2, uint8_t d3, uint8_t cp): LiquidCrystal(rs, enable, d0, d1, d2, d3){
  codepage = cp;
}

// converts utf-8 string to cp1251
// author V. Kirievskiy - vek2006_73@@mail.ru
String RobotClass_LiquidCrystal::utf8rus(String source){
  int i,k;
  String target;
  unsigned char n;
  char m[2] = { '0', '\0' };
...
size_t RobotClass_LiquidCrystal::print( const String &s ){
  if( codepage == CP_UTF8 ){
    return LiquidCrystal::print( convertCyr( utf8rus( s )));
  } else {
    return LiquidCrystal::print( convertCyr( s ));
  }
}

// converts cp1251 String to LCD codepage String
String RobotClass_LiquidCrystal::convertCyr( const String &s ){
  String target = s;
...
uint8_t RobotClass_LiquidCrystal::getCharCyr( uint8_t ch ){
  char rch = ch;
  switch (ch){
